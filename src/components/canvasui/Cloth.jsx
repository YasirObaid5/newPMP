/* eslint-disable react-refresh/only-export-components, react-hooks/set-state-in-effect, no-empty */
"use client";
...
export function createCloth(elements, options = {}) {
  ...
  const fallbackImage = htmlInCanvas ? null : content.querySelector("img");
  ...
  capture = () => {
    try {
      ...
      if (htmlInCanvas) {
      ...
      } else if (fallbackImage?.complete && fallbackImage.naturalWidth) {
        ...
        sourceCtx.drawImage(
          fallbackImage,
          (source.width - width) * 0.6,
          (source.height - height) * 0.5,
          width,
          height
        );
      } else {
  ...
  function syncCanvasSize() {
    ...
    contentMaxX = Math.min(1, Math.max(0.05, content.clientWidth / Math.max(wrapper.clientWidth, 1)));
  ...
  function composeOffsets(width, height) {
    const cellW = width / SEG;
    const cellH = height / SEG;
    const mid = SEG >> 1;
  ...
  function composeVertices(width, height) {
  ...
  function start() {
    if (!htmlInCanvas && !fallbackImage) return;
  ...
  fallbackImage?.addEventListener("load", capture);
  ...
  return {
    ...
    destroy() {
      ...
      fallbackImage?.removeEventListener("load", capture);
